...
export const stopLanyuPolling=()=>{if(lanyuPoller)clearInterval(lanyuPoller);lanyuPoller=null;pollInFlight=false;};
export function startLanyuPolling(refresh){stopLanyuPolling();lanyuPoller=setInterval(async()=>{if(document.hidden||pollInFlight)return;pollInFlight=true;try{await refresh();}catch(error){console.error("Lanyu refresh",error);}finally{pollInFlight=false;}},CONFIG.POLLING_INTERVAL);}
